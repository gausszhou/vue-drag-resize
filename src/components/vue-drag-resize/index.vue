...
export default {
  ...
  methods: {
    ...
    moveHorizontally(val) {
      const [deltaX, _] = snapToGrid(this.grid, val, this.top, this.scale); // eslint-disable-line no-unused-vars
      const left = restrictToBounds(deltaX, this.bounds.minLeft, this.bounds.maxLeft);
      this.left = left;
      this.right = this.parentWidth - this.width - left;
    },
    moveVertically(val) {
      const [_, deltaY] = snapToGrid(this.grid, this.left, val, this.scale); // eslint-disable-line no-unused-vars
    ...
    changeWidth(val) {
      const [newWidth, _] = snapToGrid(this.grid, val, 0, this.scale); // eslint-disable-line no-unused-vars
    ...
    changeHeight(val) {
      const [_, newHeight] = snapToGrid(this.grid, 0, val, this.scale); // eslint-disable-line no-unused-vars
    ...
    formatTransformVal(string) {
      let [left, top, rotate] = string.match(/[\d|\.]+/g);
